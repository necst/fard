#ifndef FILTERS_ERODE_HPP_
#define FILTERS_ERODE_HPP_
...
uint8_t compute_erode(hls::Window<KERN_SIZE,KERN_SIZE,uint8_t> & window){
	...
	for(int i=0;i<KERN_SIZE;i++) {
		for(int j=0;j<KERN_SIZE;j++) {
            int val = window.getval(i,j);
            sum += val > 127 ? 1 : 0;
        }
    }

    if(sum == KERN_SIZE*KERN_SIZE) return 255;
    else return 0;
	
}

template<int IMGH, int IMGW, int KERN_SIZE>
void erode_filter(unsigned char *input, unsigned char *output){
		...
		cols:for(register int x=0; x<IMGW;x++){
			...
			if(pixCount<WAIT_TIME){
				...
				if(y>=KERN_SIZE-1 && x>=KERN_SIZE-1){
					out = compute_erode<KERN_SIZE>(pixels);
				}else{
